...
// Update the loginUser function to handle the mock user
  ...
  try {
    ...
    if (data.user) {
      ...
      if (preferencesError || !preferencesData) {
        console.log("Creating preferences for user:", data.user.id)

        const { error: createPrefsError } = await supabase.from("preferences").insert({
        ...
        if (createPrefsError) {
          console.error("Error creating preferences:", createPrefsError)
        }
      }
    }
...
export async function getUserProfile() {
  ...
  try {
    ...
    // If profile was not found, use user data directly
